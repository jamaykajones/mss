...
import (
	...
	"github.com/jamaykajones/mss/webapp/viewmodel"
)
...
func (s shop) handleShop(w http.ResponseWriter, r *http.Request) {
	categoryPattern, _ := regexp.Compile(`/shop/(\d+)`)       //looking for /shop/ and ends with a number
	matches := categoryPattern.FindStringSubmatch(r.URL.Path) //if match found looking for a category page
	if len(matches) > 0 {
		categoryID, _ := strconv.Atoi(matches[1])
		s.handleCategory(w, r, categoryID) //modified handle
	} else {
